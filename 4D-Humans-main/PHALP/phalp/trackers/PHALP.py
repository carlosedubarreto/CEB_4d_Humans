...
import cv2
import joblib
#addicionado 2024-04-21 (para retiar o conversor to execute no addon)
import pickle
#####
import numpy as np
import torch
...
class PHALP(nn.Module):
    ...
    def track(self):
        ...
        #addicionado 2024-04-21 (para retiar o conversor to execute no addon)
        pkl_path_pickle = self.cfg.video.output_dir + '/results/' + self.cfg.track_dataset + "_" + str(self.cfg.video_seq)+'_converted' + '.pkl'
        ######
        ...
        try: 
            ...
            #addicionado 2024-04-21 (para retirar o conversor to execute no addon)
            with open(pkl_path_pickle, 'wb') as handle:
                pickle.dump(final_visuals_dic, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def cached_download_from_drive(self, additional_urls=None):
        ...
        path_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(path_addon))))
        ...
        if not os.path.exists(smpl_path):
            ...
            os.system('"'+os.path.join(path_root,'wget')+'" https://github.com/classner/up/raw/master/models/3D/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl')

            convert_pkl('basicModel_neutral_lbs_10_207_0_v1.0.0.pkl')
            # os.system('rm basicModel_neutral_lbs_10_207_0_v1.0.0.pkl')
            copyfile('basicModel_neutral_lbs_10_207_0_v1.0.0_p3.pkl',smpl_path)
            # copyfile('basicModel_neutral_lbs_10_207_0_v1.0.0_p3.pkl',smpl_path2)
            # os.system('mv basicModel_neutral_lbs_10_207_0_v1.0.0_p3.pkl ' + smpl_path)
        
        if not os.path.exists(smpl_path2):
            os.makedirs(os.path.join(CACHE_DIR, "4DHumans/data/smpl"), exist_ok=True)
            if not os.path.exists('basicModel_neutral_lbs_10_207_0_v1.0.0_p3.pkl'):
                os.system('"'+os.path.join(path_root,'wget')+'" https://github.com/classner/up/raw/master/models/3D/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl')
